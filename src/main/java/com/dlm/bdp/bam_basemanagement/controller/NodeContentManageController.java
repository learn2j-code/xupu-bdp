...
@Controller
@RequestMapping("nodecontent")
public class NodeContentManageController {
	@Autowired
	NodeContentService service;	
	...
	public @ResponseBody ResponseEntity findAll(){
		...
		List<NodeContentWithBLOBs> list = service.list();
		data.put("list", list);
	...
	public @ResponseBody ResponseEntity findByPage(@RequestBody RequestEntity requestEntity){
		...
		List<NodeContentWithBLOBs> list = service.findListByCondition(requestEntity.getNodeContent());
	...
	public @ResponseBody ResponseEntity findById(@RequestBody RequestEntity requestEntity){
		...
		NodeContent record = service.get(requestEntity.getId());
	...
	public @ResponseBody ResponseEntity add(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			NodeContentWithBLOBs nodeContent = requestEntity.getNodeContent();
			service.add(nodeContent);
			Map<String, Object> data = new HashMap<String, Object>();
			
			data.put("record", nodeContent);
			responseEntity.setData(data);
		} catch (Exception e) {
	...
	public @ResponseBody ResponseEntity update(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			service.update(requestEntity.getNodeContent());
		} catch (Exception e) {
			responseEntity.setSuccess(CommonConstant.RESPONSE_FAIL);
			responseEntity.setErrorMsg(e.getMessage());
		}
